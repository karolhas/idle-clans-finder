...
export default function AgilityItems() {
  ...
  // Get the agility items
  const agilityItems = (SKILL_ITEMS_MAP[currentSkill] as SkillItem[]) || [];

  // Sort items based on selected criterion
  const sortedItems = [...agilityItems].sort((a, b) => {
  ...
  // Find current level based on XP to determine which tasks the player can do
        ...
        <table className="min-w-full divide-y divide-white/5">
          ...
          <tbody className="divide-y divide-white/5">
            {sortedItems.map((item: SkillItem) => {
              const canDo = currentLevel >= item.level;
              ...
              return (
                <tr
                  ...
                  className={`
                                ...
                                `}
                  onClick={() => canDo && handleSelectItem(item.name)}
                >
                  ...
                  <td
                    className={`px-3 py-3 whitespace-nowrap text-sm font-mono ${
                      !canDo ? "text-red-400" : "text-emerald-400"
                    }`}
                  >
                    {item.level}
                  </td>
