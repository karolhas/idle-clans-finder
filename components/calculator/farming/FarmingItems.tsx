...
export default function FarmingItems() {
  ...
  // Get the farming items
  const farmingItems = (SKILL_ITEMS_MAP[currentSkill] as SkillItem[]) || [];

  // Sort items based on selected criterion
  const sortedItems = [...farmingItems].sort((a, b) => {
  ...
  // Find current level based on XP to determine which items the player can grow
    ...
    <div className="bg-black/70 p-6 rounded-2xl border-2 border-white/10 relative backdrop-blur-md shadow-xl hover:border-teal-500/50 hover:shadow-teal-900/20 transition-all duration-300 group mb-6">
        ...
        <table className="min-w-full divide-y divide-white/5">
          ...
          <tbody className="divide-y divide-white/5">
            {sortedItems.map((item: SkillItem) => {
              const canGrow = currentLevel >= item.level;
              ...
              return (
                <tr
                  ...
                  className={`
                                ...
                                `}
                  onClick={() => canGrow && handleSelectItem(item.name)}
                >
                  ...
                  <td
                    className={`px-3 py-3 whitespace-nowrap text-sm font-mono ${
                      !canGrow ? "text-red-400" : "text-emerald-400"
                    }`}
                  >
                    {item.level}
                  </td>
      ...
      {farmingItems.length === 0 && (
        <div className="text-center text-gray-400 py-8 italic bg-black/20 rounded-lg mt-2">
          No farming crops available yet.
        </div>
      )}
    </div>
  );
