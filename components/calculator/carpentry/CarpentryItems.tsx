...
// Extended interface to include goldCost for carpentry items
interface CarpentryItem extends SkillItem {
    goldCost: number;
}

export default function CarpentryItems() {
    ...
    // Get the carpentry items
    const carpentryItems =
        (SKILL_ITEMS_MAP[currentSkill] as CarpentryItem[]) || [];

    // Sort items based on selected criterion
    const sortedItems = [...carpentryItems].sort((a, b) => {
    ...
    // Find current level based on XP to determine which items the player can craft
        ...
        <div className="bg-[#001010] rounded-lg p-4 mb-4">
            ...
            <div className="max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                <table className="min-w-full divide-y divide-gray-700">
                    ...
                    <tbody className="bg-[#001010] divide-y divide-gray-800">
                        {sortedItems.map((item: CarpentryItem) => {
                            ...
                            return (
                                ...
                                `}
                                    onClick={() =>
                                        canCraft && handleSelectItem(item.name)
                                    }
                                >
            ...
            {carpentryItems.length === 0 && (
                <div className="text-center text-gray-400 py-4">
                    No carpentry items available yet.
                </div>
            )}
        </div>
    );
