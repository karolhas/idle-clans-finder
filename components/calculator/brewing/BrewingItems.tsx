...
export default function BrewingItems() {
  ...
  // Get the brewing items
  const brewingItems = (SKILL_ITEMS_MAP[currentSkill] as SkillItem[]) || [];

  // Sort items based on selected criterion
  const sortedItems = [...brewingItems].sort((a, b) => {
  ...
  // Find current level based on XP to determine which potions the player can brew
        ...
        <table className="min-w-full divide-y divide-white/5">
          ...
          <tbody className="divide-y divide-white/5">
            {sortedItems.map((item: SkillItem) => {
              const canBrew = currentLevel >= item.level;
              ...
              return (
                <tr
                  ...
                  className={`
                                ...
                                `}
                  onClick={() => canBrew && handleSelectItem(item.name)}
                >
                  ...
                  <td
                    className={`px-3 py-3 whitespace-nowrap text-sm font-mono ${
                      !canBrew ? "text-red-400" : "text-emerald-400"
                    }`}
                  >
                    {item.level}
                  </td>
