...
export default function CookingItems() {
  ...
  // Get the cooking items
  const cookingItems = (SKILL_ITEMS_MAP[currentSkill] as SkillItem[]) || [];

  // Group items by category
  const groupedItems: Record<string, SkillItem[]> = {};
    ...
    <div className="bg-black/70 p-6 rounded-2xl border-2 border-white/10 relative backdrop-blur-md shadow-xl hover:border-teal-500/50 hover:shadow-teal-900/20 transition-all duration-300 group mb-6">
          ...
          <tbody className="divide-y divide-white/5">
            {Object.keys(groupedItems).map((category) => (
              <React.Fragment key={category}>
                ...
                {groupedItems[category].map((item: SkillItem) => {
                  const canCook = currentLevel >= item.level;
                  ...
                  return (
                    <tr
                      ...
                      className={`
                                            ...
                                            `}
                      onClick={() => canCook && handleSelectItem(item.name)}
                    >
                      ...
                      <td
                        className={`px-3 py-3 whitespace-nowrap text-sm font-mono ${
                          !canCook ? "text-red-400" : "text-emerald-400"
                        }`}
                      >
                        {item.level}
                      </td>
      ...
      {cookingItems.length === 0 && (
        <div className="text-center text-gray-400 py-8 italic bg-black/20 rounded-lg mt-2">
          No cooking items available yet.
        </div>
      )}
    </div>
  );
